...
/**
 * Public screen: mission cards revealed anonymously (shuffled), then the
 * outcome. A local "show results" gate holds the cards face-down until the
 * table is ready for the beat.
 */
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {revealed
          ...
          : display.map((_, i) => (
              <div
                key={i}
                className="flex h-20 w-14 animate-pop items-center justify-center rounded-field border border-line-strong bg-raised text-2xl font-bold text-faint"
                style={{ animationDelay: `calc(${i} * var(--m-fast))` }}
              >
                ?
              </div>
      ...
      {revealed ? (
        <>
          <p
            ...
            style={{ animationDelay: `calc(${total} * var(--m-fast))` }}
          >
            {lastMission.success ? lex.missionReveal.success : lex.missionReveal.failure}
          </p>
          <p
            className="animate-fade font-mono text-xs uppercase tracking-label text-faint"
            style={{ animationDelay: `calc(${total} * var(--m-fast))` }}
          >
            {failsLine}
        ...
        <>
          <p className="font-mono text-xs uppercase tracking-label text-faint">
            {lex.missionReveal.allIn}
          </p>
          <Button className="w-full max-w-sm" onClick={() => setRevealed(true)}>
